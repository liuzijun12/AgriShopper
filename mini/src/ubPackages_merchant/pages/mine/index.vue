<template>
  <view class="app-container">
    ...
    <!-- 商户端导航栏 -->
    <MerchantTabBar :current-path="currentPath" />
  </view>
</template>
...
const currentPath = ref<string>('/ubPackages_merchant/pages/mine/index');
import { onShow } from "@dcloudio/uni-app";
import { useToast } from "wot-design-uni";
import { useUserStore } from "@/ubPackages_merchant/store/modules/user.store";
...
// 商户页面假设用户已登录，如果没有登录信息则提供默认值
const userInfo = computed(() => userStore.userInfo || {
  id: "merchant_001",
  username: "merchant001", 
  nickname: "商户用户",
  avatar: "/static/images/default-avatar.png", // 使用存在的默认头像
  phone: "",
  email: ""
});

const isLogin = computed(() => true); // 商户页面默认已登录
...
const navigateToLoginPage = () => {
  ...
  uni.navigateTo({
    url: `/pages/login/login?redirect=${encodeURIComponent(currentPagePath)}`,
  });
};
...
const navigateToProfile = () => {
  uni.navigateTo({ url: "/ubPackages_merchant/pages/mine/profile/index" });
};

// 常见问题
const navigateToFAQ = () => {
  uni.navigateTo({ url: "/ubPackages_merchant/pages/mine/faq/index" });
};

// 关于我们
const navigateToAbout = () => {
  uni.navigateTo({ url: "/ubPackages_merchant/pages/mine/about/index" });
};

// 设置
const navigateToSettings = () => {
  uni.navigateTo({ url: "/ubPackages_merchant/pages/mine/settings/index" });
};

// 问题反馈
const handleQuestionFeedback = () => {
  uni.navigateTo({ url: "/ubPackages_merchant/pages/mine/feedback/index" });
};
