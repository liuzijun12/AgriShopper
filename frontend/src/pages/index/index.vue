...
export default {
  ...
  methods: {
    ...
    handleCategoryClick(item) {
      ...
      // 跳转到对应分类的商品列表页面
      uni.navigateTo({
        url: `/subPackages_user/pages/productList/productList?categoryId=${item.id}&categoryName=${item.name}`
      });
    },
    ...
    handleMoreClick() {
      uni.navigateTo({
        url: '/subPackages_user/pages/category/category'
      })
    },
    ...
    async getCategoryList() {
      try {
        ...
        console.log('获取分类数据成功:', categories)
      } catch (error) {
    ...
    goToProduct(productId) {
      ...
      // 暂时跳转到商品列表页面作为示例
      uni.navigateTo({
        url: '/subPackages_user/pages/productList/productList?categoryId=1&categoryName=推荐商品'
      })
    },
    ...
    async getProductList() {
      try {
        const products = await ProductAPI.getRecommendProducts(4)
        this.productList = products || []
        console.log('获取推荐商品成功:', products)
      } catch (error) {
    ...
    handleSearchClick() {
      ...
      // 跳转到搜索页面，不传递任何参数
      uni.navigateTo({
        url: '/subPackages_user/pages/search/search'
      })
    },
