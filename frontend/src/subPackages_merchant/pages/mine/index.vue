...
import { checkPageAccess, getCurrentUser, routeLogout } from '@/utils/routeGuard';
...
// 使用本地存储的用户信息
const localUserInfo = ref(getCurrentUser());
const userInfo = computed(() => localUserInfo.value || userStore.userInfo);
...
onLoad(() => {
  ...
  // 更新本地用户信息
  localUserInfo.value = getCurrentUser();
  uni.hideHomeButton();
});

// 重写返回按钮行为
const onBackPress = () => {
  console.log('商户我的页面 - 拦截返回按钮')
  // 直接跳转到商户首页
  uni.redirectTo({
  ...
  uni.showModal({
    ...
    success: function (res) {
      if (res.confirm) {
        ...
        setTimeout(() => {
          uni.reLaunch({
            url: '/pages/index/index'  // 改为跳转到登录页
          });
        }, 1000);
      }
    },
...
const navigateToProfile = () => {
  ...
  uni.navigateTo({ url: "/subPackages_merchant/pages/mine/profile/index" });
};

// 常见问题
const navigateToFAQ = () => {
  uni.navigateTo({ url: "/subPackages_merchant/pages/mine/faq/index" });
};

// 关于我们
const navigateToAbout = () => {
  uni.navigateTo({ url: "/subPackages_merchant/pages/mine/about/index" });
};

// 设置
const navigateToSettings = () => {
  uni.navigateTo({ url: "/subPackages_merchant/pages/mine/settings/index" });
};

// 问题反馈
const handleQuestionFeedback = () => {
  uni.navigateTo({ url: "/subPackages_merchant/pages/mine/feedback/index" });
};
...
onShow(() => {
  ...
  if (pages.length > 0) {
    const currentPage = pages[pages.length - 1];
    if (currentPage.route.includes("subPackages_merchant/pages/mine")) {
      // 通过事件通知 tabbar 布局更新状态
      uni.$emit("updateTabbar", "manager-mine");
    }
  }
});
</script>
